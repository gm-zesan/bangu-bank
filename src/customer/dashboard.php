<?php
    ...
    $allTransaction = file_exists($file) ? json_decode(file_get_contents($file), true) : [];

    $loggedInUserEmail = $session->get('user');

    $transactions = array_filter($allTransaction, function($transaction) use ($loggedInUserEmail) {
        return $transaction['email'] === $loggedInUserEmail;
    });
